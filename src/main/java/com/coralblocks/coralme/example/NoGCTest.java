...
/**
 * <p>
 * Manual garbage-collection smoke test for a repeated, capacity-stable order
 * book workload.
 * </p>
 * <p>
 * Build with <code>mvn package</code>, then run:
 * </p>
 *
 * <p>
 * <code>java -Xlog:gc -Xms32m -Xmx64m -cp target/coralme-all.jar
 * com.coralblocks.coralme.example.NoGCTest false 1000000</code>
 * </p>
 *
 * <p>
 * The normal run must show no collection pauses. Pass <code>true</code> as the
 * first argument to enable a negative control that deliberately creates garbage
 * and produces collection activity.
 * </p>
 */
public class NoGCTest {
	...
	private static final boolean USE_BAD_SYSTEM_OUT_PRINT = false; // turn this on and you will see a lot of garbage
	...
	public static void main(String[] args) {
		...
		for (int i = 1; i <= iterations; i++) {
			...
			if (createGarbage) {
				...
				for (int x = 0; x < 10; x++)
					garbageSink = sb.toString(); // this produces garbage
			}
		...
		if (listener.rejectedOrders != expectedRejectedOrders) {
			throw new IllegalStateException(
					"Expected " + expectedRejectedOrders + " rejected orders but found " + listener.rejectedOrders);
		}
